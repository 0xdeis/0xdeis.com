...
There's the [`net/http`](https://pkg.go.dev/net/http) package in the standard library, but theres also 3rd party libraries:
...
They all have different features and tradeoffs, but we wil be using the built-in `net/http` because it is one less thing to learn,
...
> [`http.ListenAndServe(":8080", nil)`](https://pkg.go.dev/net/http#ListenAndServe) listens on the TCP network address 8080 and then calls Serve with handler to handle requests on incoming connections. Accepted connections are configured to enable TCP keep-alives.
> 
...
You need to stop (ctrl+c) and restart the server whenevery you make changes to your code. We will address this soon.
...
Go supports annonymous functions, which leads to locality of behavior, something that is useful when developing larger projects.
...
We can introduce some state in the form of a global varialbe.
...
This because we specificed the POST method for the `/count/increment` and `/count/reset`
