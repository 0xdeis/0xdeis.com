...
This might not make much sense on its own, so let's use a basic command as an example: ls. ls stands for list and all it does is list the contents of a directory (this is the same thing as a folder on windows) and print it to the terminal. 
