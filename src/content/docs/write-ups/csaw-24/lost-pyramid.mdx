...
t   token = jwt.encode(payload, PRIVATE_KEY, algorithm="EdDSA")
...
The /scarab_room route is vulnrable to SSTI via the `name` parameter. However, it does it processed before the SSTI. Only alphanumeric characters and "{" and "}" are allowed.
...
We can leak two out of the three variables we need, but `PRIVATE_KEY` has an "_", so we can't leak it via SSTI.
...
This next part took me a while to figure out. I tried many different jinja2 (the templating language) [SSTI tricks](https://book.hacktricks.xyz/pentesting-web/ssti-server-side-template-injection/jinja2-ssti)
...
The [jwt.io](https://jwt.io/) website is an easy way to play around with encoding and decoding jwts.
