---
title: Playing on the Backcourts
author: AlexanderHOtt
---

## Challenge

> yadayada playing tennis like pong yadayada someone's cheating yadayada at
> least the leaderboard is safe!
>
> -- smallfoot

```py title="app_public.py"
...
Reading through app_public.py,
`safetytime = 'csawctf{i_look_different_in_prod}'` and the `/get_eval` route
stand out.
...
You should _never_ use `exec` or `eval`, especially when the input comes from an
untrusted source (e.g. a POST request).
